...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .onReceive(ChatHistoryManager.shared.$histories) { histories in
            ...
            // Note: Don't auto-exit when history is missing - session may have ended
            // but user is still viewing the chat. Only exit on explicit user action.
        }
        .onReceive(sessionMonitor.$instances) { sessions in
            if let updated = sessions.first(where: { $0.sessionId == sessionId }) {
                ...
                // Session found, mark as active
                if sessionEnded {
                    sessionEnded = false
                }
            } else if hasLoadedOnce && !sessionEnded {
                // Session is no longer in the instances list (ended)
                // Keep showing the chat history, just mark as ended
                sessionEnded = true
            }
        }
    ...
    /// The menu options from the permission context
    private var menuOptions: [MenuOption] {
        session.activePermission?.menuOptions ?? []
    }

    /// The prompt text from the permission context
    private var menuPrompt: String {
        session.activePermission?.promptText ?? "Allow \(session.phase.approvalToolName ?? "tool")?"
    }

    private func approvalBar(tool: String) -> some View {
        DynamicChatApprovalBar(
            tool: tool,
            ...
            onSelectOption: { option in
                sessionMonitor.selectMenuOption(sessionId: sessionId, option: option)
            }
        )
    }
...
/// A button with magical hover and click effects for approval actions
struct ApprovalButton: View {
    ...
    init(title: String, foregroundColor: Color, backgroundColor: Color, tooltip: String? = nil, action: @escaping () -> Void) {
        self.title = title
    ...
    private var glowColor: Color {
        backgroundColor.opacity(isHovered ? 0.6 : 0)
    }
    ...
    var body: some View {
        Button {
            ...
            Text(title)
                ...
                .shadow(color: glowColor, radius: isHovered ? 8 : 0, x: 0, y: 0)
...
struct ChatApprovalBar: View {
    ...
    var onApproveWithFolderAccess: () -> Void = { }
    var onApproveAndRememberMCP: () -> Void = { }
    ...
    /// Formatted display for the command/action
    ...
    /// Header text based on approval type
    private var headerText: String {
        switch approvalType {
        case .standard:
            return "Approve \(MCPToolFormatter.formatToolName(tool))?"
        case .planEntry:
            return "Enter plan mode?"
        case .planExecution:
            return "Execute plan?"
        case .folderCommandAccess:
            return "Allow folder/command access?"
        case .mcpPersistentPermission(let pluginName):
            return "Allow \(pluginName)?"
        }
    }
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        if fullCommand != nil && approvalType == .standard {
                            Button {
                    ...
                    // Command preview (truncated unless expanded) - only for standard approval
                    if approvalType == .standard, let cmd = displayCommand {
                        Text(cmd)
                            .font(.system(size: 11, design: .monospaced))
                            .foregroundColor(TerminalColors.amber)
                            .lineLimit(isExpanded ? nil : 2)
                            .textSelection(.enabled)
                    } else if approvalType == .planEntry {
                        Text("Claude will explore and design before coding")
                            .font(.system(size: 11))
                            .foregroundColor(.white.opacity(0.5))
                    } else if approvalType == .planExecution {
                        Text("Ready to implement the plan")
                            .font(.system(size: 11))
                            .foregroundColor(.white.opacity(0.5))
                    }
                }
                ...
                if approvalState == .pending {
                    ...
                    // (since we can't send special options without tmux)
    ...
    private var standardApprovalButtons: some View {
        Group {
            ...
            ApprovalButton(
                ...
                tooltip: "3. No – Reject this action"
            ) {
                withAnimation(.easeInOut(duration: 0.2)) {
            ...
            ApprovalButton(
                ...
                tooltip: "1. Yes – Approve this action"
            ) {
                withAnimation(.easeInOut(duration: 0.2)) {
            ...
            if isInTmux {
                ApprovalButton(
                    ...
                    tooltip: "2. Yes, and don't ask again for this session"
                ) {
                    withAnimation(.easeInOut(duration: 0.2)) {
    ...
    private var planEntryButtons: some View {
        Group {
            ...
            ApprovalButton(
                ...
                tooltip: "Enter plan mode - Claude will explore before coding"
            ) {
                withAnimation(.easeInOut(duration: 0.2)) {
                    approvalState = .approved
                }
    ...
    private var folderAccessButtons: some View {
        Group {
            ...
            ApprovalButton(
                ...
                foregroundColor: .white.opacity(0.8),
                backgroundColor: Color.white.opacity(0.1),
                tooltip: "3. No – Reject this request"
            ) {
                withAnimation(.easeInOut(duration: 0.2)) {
                    approvalState = .denied
                }
            ...
            if isInTmux {
                ApprovalButton(
                    ...
                    tooltip: "2. Grant persistent folder/command access"
                ) {
                    withAnimation(.easeInOut(duration: 0.2)) {
                        approvalState = .approvedForSession
                    }
        ...
        Group {
            ...
            if isInTmux {
                ApprovalButton(
                    ...
                    tooltip: "2. Don't ask again for this MCP command"
                ) {
                    withAnimation(.easeInOut(duration: 0.2)) {
                        approvalState = .approvedForSession
                    }
...
#Preview("Chat Approval Bar") {
    VStack(spacing: 20) {
        Text("Standard (with tmux)").foregroundColor(.white.opacity(0.5))
        ChatApprovalBar(
            tool: "bash",
            toolInput: "command: rm -rf node_modules",
            fullCommand: "rm -rf node_modules && npm install",
        ...
        Text("Standard (no tmux)").foregroundColor(.white.opacity(0.5))
        ChatApprovalBar(
            tool: "write_file",
            toolInput: nil,
            fullCommand: "/path/to/file.txt",
            approvalType: .standard,
            isInTmux: false,
            onApprove: { print("Approved once") },
            onApproveForSession: { },
        ...
        Text("Plan Entry").foregroundColor(.white.opacity(0.5))
        ChatApprovalBar(
            tool: "EnterPlanMode",
            toolInput: nil,
            fullCommand: nil,
        ...
        Text("Plan Execution").foregroundColor(.white.opacity(0.5))
        ChatApprovalBar(
            tool: "ExitPlanMode",
            toolInput: nil,
            fullCommand: nil,
            ...
            onEnterPlanMode: { },
            onSkipPlanMode: { },
            onApprovePlanAutoAccept: { print("Auto-accept") },
            onApprovePlanManual: { print("Manual") }
        )
    }
    .frame(width: 600)

...
struct NotchView: View {
    ...
    @StateObject private var spaceTransitionDetector = SpaceTransitionDetector.shared
    ...
    @State private var isSettingsPressed: Bool = false
    @State private var isSpaceTransitioning: Bool = false
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                notchLayout
                    ...
                    // Click handling is done by global event monitor in NotchViewModel
                    // which properly toggles open/close state
            }
        }
        .opacity(isVisible && !isSpaceTransitioning ? 1 : 0)
        .animation(.easeOut(duration: 0.1), value: isSpaceTransitioning)
        .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
        .preferredColorScheme(.dark)
        .onAppear {
            sessionMonitor.startMonitoring()
            PluginRegistry.shared.startHealthChecks()
            // On non-notched devices, keep visible so users have a target to interact with
            if !viewModel.hasPhysicalNotch {
                isVisible = true
            }
        }
        .onReceive(spaceTransitionDetector.$isTransitioning) { transitioning in
            isSpaceTransitioning = transitioning
        }
    ...
    private var openedHeaderContent: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                ZStack(alignment: .topTrailing) {
                    Image(systemName: viewModel.contentType == .menu ? "xmark" : "gearshape")
                        ...
                        .foregroundColor(.white.opacity(isSettingsHovered ? 0.9 : 0.4))
                        .frame(width: 22, height: 22)
                        .background(
                            Circle()
                                .fill(Color.white.opacity(isSettingsHovered ? 0.1 : 0))
                                .scaleEffect(isSettingsHovered ? 1.0 : 0.8)
                        )
                        .scaleEffect(isSettingsPressed ? 0.85 : (isSettingsHovered ? 1.1 : 1.0))
                        .contentShape(Rectangle())
            ...
            .help(viewModel.contentType == .menu ? "Close Settings" : "Open Settings")
        }
    }
    ...
    private func handleStatusChange(from oldStatus: NotchStatus, to newStatus: NotchStatus) {
        ...
        case .opened, .popping:
            isVisible = true
            // Start keyboard shortcuts when opened
            keyboardHandler.startMonitoring(sessionMonitor: sessionMonitor)
        ...
        case .closed:
            // Stop keyboard shortcuts when closed
    ...
    private func handlePendingSessionsChange(_ sessions: [SessionState]) {
        ...
        if !newPendingIds.isEmpty && viewModel.status == .closed {
            ...
            let shouldExpand = AppSettings.alwaysExpandForApprovals ||
            ...
            // Play notification sound for new pending approvals
            if AppSettings.playSoundForApprovals,
               let soundName = AppSettings.notificationSound.soundName {
                // Get newly pending sessions for focus check

...
actor SessionStore {
    ...
    nonisolated static let logger = Logger(subsystem: "com.deliciousisland", category: "Session")
    ...
    func process(_ event: SessionEvent) async {
        ...
        case .permissionApprovedForSession(let sessionId, let toolUseId):
            await processPermissionApprovedForSession(sessionId: sessionId, toolUseId: toolUseId)
        ...
        case .menuCaptured(let sessionId, let toolUseId, let menu):
            await processMenuCaptured(sessionId: sessionId, toolUseId: toolUseId, menu: menu)
    ...
    private func processHookEvent(_ event: HookEvent) async {
        ...
        _ = sessions[sessionId] == nil  // Track if this is a new session
    ...
    private func processPermissionApproved(sessionId: String, toolUseId: String) async {
        ...
        if let nextPending = findNextPendingTool(in: session, excluding: toolUseId) {
            ...
            let menu = ToolMenuMapper.generateMenu(toolName: nextPending.name, toolInput: nil, permissionMode: nil)
    ...
    private func processPermissionApprovedForSession(sessionId: String, toolUseId: String) async {
        guard var session = sessions[sessionId] else { return }

        // Update tool status in chat history first
        updateToolStatus(in: &session, toolId: toolUseId, status: .running)

        // Same logic as single approval - check if there are other tools waiting
        if let nextPending = findNextPendingTool(in: session, excluding: toolUseId) {
            // Another tool is waiting - stay in waitingForApproval with that tool's context
            let menu = ToolMenuMapper.generateMenu(toolName: nextPending.name, toolInput: nil, permissionMode: nil)
            let newPhase = SessionPhase.waitingForApproval(PermissionContext(
            ...
            if session.phase.canTransition(to: newPhase) {
                session.phase = newPhase
                Self.logger.debug("Switched to next pending tool (session approval): \(nextPending.id.prefix(12), privacy: .public)")
            }
        } else {
            // No more pending tools - transition to processing
            if case .waitingForApproval(let ctx) = session.phase, ctx.toolUseId == toolUseId {
        ...
        sessions[sessionId] = session
        Self.logger.info("Permission approved for rest of session: \(sessionId.prefix(8), privacy: .public)")
    }
    ...
    /// Update the menu for a permission request after tmux capture
    private func processMenuCaptured(sessionId: String, toolUseId: String, menu: ExtractedMenu) async {
        guard var session = sessions[sessionId] else { return }

        // Only update if we're still waiting for approval on this tool
        guard case .waitingForApproval(let ctx) = session.phase,
              ctx.toolUseId == toolUseId else {
            Self.logger.debug("Menu captured but tool \(toolUseId.prefix(12), privacy: .public) no longer active")
            return
        }

        // Create new context with captured menu
        let newContext = PermissionContext(
        ...
        session.phase = .waitingForApproval(newContext)
        sessions[sessionId] = session

        Self.logger.info("Updated menu from tmux capture: \(menu.options.count) options for \(toolUseId.prefix(12), privacy: .public)")
    }

...
private let logger = Logger(subsystem: "com.deliciousisland", category: "Hooks")
...
struct HookEvent: Codable, Sendable {
    ...
    init(sessionId: String, cwd: String, event: String, status: String, pid: Int?, tty: String?, tool: String?, toolInput: [String: AnyCodable]?, toolUseId: String?, notificationType: String?, message: String?, transcriptPath: String? = nil) {
    ...
    var sessionPhase: SessionPhase {
        ...
        case "waiting_for_approval":
            ...
            let toolInputDict: [String: Any]? = toolInput?.mapValues { $0.value }
            let menu = ToolMenuMapper.generateMenu(
                toolName: tool,
                toolInput: toolInputDict,
                permissionMode: nil,  // TODO: Pass permission_mode from hook
                message: message
            )
...
struct HookResponse: Codable {
    let decision: String // "allow", "allow_for_session", "deny", or "ask"
    let reason: String?
}
...
class HookSocketServer {
    ...
    static let socketPath = "/tmp/delicious-island.sock"

    private var serverSocket: Int32 = -1
    private var acceptSource: DispatchSourceRead?
    private var eventHandler: HookEventHandler?
    private var permissionFailureHandler: PermissionFailureHandler?
    private let queue = DispatchQueue(label: "com.deliciousisland.socket", qos: .userInitiated)
    ...
    /// Close the socket for a pending permission without sending a response.
    /// This causes the Python hook to exit without output, letting Claude show its native menu.
    /// Used for "Session" approval which requires tmux key sending instead of hook response.
    func closeWithoutResponse(toolUseId: String) {
        queue.async { [weak self] in
            self?.closeSocketWithoutResponse(toolUseId: toolUseId)
        }
    }

    private func closeSocketWithoutResponse(toolUseId: String) {
        permissionsLock.lock()
        guard let pending = pendingPermissions.removeValue(forKey: toolUseId) else {
            permissionsLock.unlock()
            logger.debug("closeWithoutResponse: No pending permission for toolUseId: \(toolUseId.prefix(12), privacy: .public)")
            return
        }
        permissionsLock.unlock()

        logger.info("Closing socket without response for \(pending.sessionId.prefix(8), privacy: .public) tool:\(toolUseId.prefix(12), privacy: .public) - will use tmux")
        close(pending.clientSocket)
    }
    ...
    private func handleClient(_ clientSocket: Int32) {
        ...
        if event.expectsResponse {
            ...
            if let eventToolUseId = event.toolUseId {
                ...
                // Generate a fallback toolUseId to ensure the permission flow continues
                // This handles race conditions where PermissionRequest arrives before PreToolUse cache
                let fallbackId = "fallback-\(event.sessionId.prefix(8))-\(Int(Date().timeIntervalSince1970 * 1000))"
                logger.warning("Permission request missing tool_use_id for \(event.sessionId.prefix(8), privacy: .public) tool:\(event.tool ?? "unknown", privacy: .public) - using fallback: \(fallbackId.prefix(20), privacy: .public)")
                toolUseId = fallbackId
            }

            logger.info("Permission request - tool:\(event.tool ?? "nil", privacy: .public) message:\(event.message ?? "nil", privacy: .public)")

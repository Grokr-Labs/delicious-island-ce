...
class NotchViewModel: ObservableObject {
    ...
    /// Whether we're in a mode where clicking inside shouldn't close (chat or menu)
    private var isInStickyMode: Bool {
        switch contentType {
        case .chat: return true
        case .menu: return true
        case .instances: return false
        }
    }
    ...
    private func handleMouseDown() {
        ...
        case .opened:
            if geometry.isPointOutsidePanel(location, size: openedSize) {
                ...
                // Clicking notch while opened - only close if NOT in sticky mode (chat or menu)
                if !isInStickyMode {
                    notchClose()
                }
            }

...
enum SessionEvent: Sendable {
    ...
    /// User approved a permission request (one-time)
    case permissionApproved(sessionId: String, toolUseId: String)

    /// User approved a permission request for the rest of the session
    case permissionApprovedForSession(sessionId: String, toolUseId: String)
    ...
    /// Menu was captured from tmux terminal (updates predictive menu with actual menu)
    case menuCaptured(sessionId: String, toolUseId: String, menu: ExtractedMenu)
...
extension SessionEvent: CustomStringConvertible {
    nonisolated var description: String {
        ...
        case .permissionApprovedForSession(let sessionId, let toolUseId):
            return "permissionApprovedForSession(session: \(sessionId.prefix(8)), tool: \(toolUseId.prefix(12)))"
        ...
        case .menuCaptured(let sessionId, let toolUseId, let menu):
            return "menuCaptured(session: \(sessionId.prefix(8)), tool: \(toolUseId.prefix(12)), options: \(menu.options.count))"

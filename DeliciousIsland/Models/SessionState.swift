...
struct SessionState: Equatable, Identifiable, Sendable {
    ...
    /// Full command or primary input (not truncated)
    var pendingFullCommand: String? {
        activePermission?.fullCommand
    }

    /// Full formatted tool input (not truncated)
    var pendingFullToolInput: String? {
        activePermission?.fullFormattedInput
    }

    /// Transcript path for the session
    var pendingTranscriptPath: String? {
        activePermission?.transcriptPath
    }
